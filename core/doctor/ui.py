# plik: core/doctor/ui.py
# Wersja 1.1 - Naprawiono automatyczne odświeżanie statusu.
...
class DoctorLiveDisplay:
    ...
    def _build_layout(self) -> Layout:
        ...
        status_table = Table.grid(padding=(0, 2), expand=True); status_table.add_column("Test Diagnostyczny", style="cyan", no_wrap=True, ratio=1); status_table.add_column("Status / Wynik", style="white", ratio=2)
        for test in self.tests: status_table.add_row(test['name'], self.statuses[test['name']])
        status_panel = Panel(Align.center(status_table), title="[bold blue]Postęp Diagnostyki[/]", border_style="blue")
        spinner_panel = Panel(Align.center(self.current_spinner), border_style="dim")
        layout = Layout(); layout.split_column(Layout(Panel(self._title_text), name="header", size=3), Layout(status_panel, name="main"), Layout(spinner_panel, name="footer", size=3))
        return layout

    def __enter__(self):
        # === POCZĄTEK POPRAWKI: Zmieniamy auto_refresh na False, będziemy odświeżać ręcznie ===
