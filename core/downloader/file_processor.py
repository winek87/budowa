...
# Wersja 1.1 - Dodano raportowanie statusu za pomocą funkcji callback.
# -*- coding: utf-8 -*-
...
from ..config import (
    DOWNLOADS_DIR_BASE, FORCED_DUPLICATES_DIR, METADATA_STRATEGY,
    THREE_DOTS_MENU_SELECTOR, DOWNLOAD_OPTION_SELECTOR,
    WAIT_FOR_SELECTOR, WAIT_FOR_DOWNLOAD_START
)
...
async def download_file_with_fallbacks(page: Page, status_callback: Callable[[str], None]) -> Download:
    """
    Próbuje pobrać plik, używając skrótu klawiszowego z fallbackiem na menu,
    i raportuje postęp za pomocą callbacku.
    """
    try:
        status_callback("Pobieranie: Próba skrótem klawiszowym (Shift+D)...")
        ...
        status_callback("Pobieranie: Shift+D nie powiódł się. Próba przez menu...")
...
async def finalize_and_move_file(
    ...
    """
    Finalizuje pobieranie, scala metadane, ustala ścieżkę, przenosi plik
    i raportuje postęp za pomocą callbacku.
    """
    status_callback("Finalizacja: Oczekiwanie na zapis pliku tymczasowego...")
    ...
    status_callback("Finalizacja: Ustalanie daty i ścieżki docelowej...")
    if final_metadata.get('expected_path'):
        ...
        status_callback("Finalizacja: Odczytywanie danych EXIF z pliku...")
    ...
    status_callback(f"Finalizacja: Przenoszenie pliku do {final_path.parent.name}...")
