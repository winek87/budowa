...
# Wersja 1.2 - Zmieniono na użycie funkcji callback do raportowania statusu.
# -*- coding: utf-8 -*-
...
from ..config import (
    ...
    WAIT_FOR_SELECTOR, WAIT_FOR_PAGE_LOAD, NAV_BLIND_CLICK_ENABLED, DIRECTION_KEY
)
from ..utils import stop_event

logger = logging.getLogger(__name__)

async def unstoppable_navigate(page: Page, current_url: str, status_callback: Callable[[str], None]) -> bool:
    """
    Niezwykle 'uparta' funkcja do nawigacji, która raportuje swój status
    za pomocą przekazanej funkcji zwrotnej (callback).
    """
    ...
    try:
        status_callback("Nawigacja: Próba standardowego kliknięcia...")
    ...
    for attempt in range(1, NAV_REFRESH_ATTEMPTS + 1):
        ...
        status_callback(f"Nawigacja: Odświeżanie strony ({attempt}/{NAV_REFRESH_ATTEMPTS})...")
        try:
            await page.reload(wait_until='load', timeout=WAIT_FOR_PAGE_LOAD * 1000)
            status_callback(f"Nawigacja: Próba kliknięcia po odświeżeniu...")
            await page.click(selector, timeout=WAIT_FOR_SELECTOR * 1000)
            await page.wait_for_url(lambda url: url != current_url, timeout=WAIT_FOR_SELECTOR * 1000)
            return True
        except PlaywrightTimeoutError:
    ...
    if NAV_BLIND_CLICK_ENABLED:
        if stop_event.is_set(): return False
        status_callback("Nawigacja: Próba za pomocą klawiatury...")
        try:
    ...
    status_callback("Nawigacja: Ostateczna próba (pełne przeładowanie)...")
    try:
        ...
        status_callback("❌ Błąd krytyczny nawigacji!")
